#!/usr/bin/env python3
"""Capture normal work-window snapshots after read-only GET CONFIG variants."""
...
class Variant:
    ...
    def cdb(self) -> tuple[int, ...]:
        return (
            0x46,
            self.rt & 0x03,
            (self.start_feature >> 8) & 0xFF,
            self.start_feature & 0xFF,
            0,
            0,
            0,
            (self.allocation >> 8) & 0xFF,
            self.allocation & 0xFF,
            0,
        )


VARIANTS: tuple[Variant, ...] = (
    Variant("get-config-current-sf0000-len0010", 0x02, 0x0000, 0x0010),
    Variant("get-config-current-sf0000-len0040", 0x02, 0x0000, 0x0040),
    Variant("get-config-current-sf0000-len00fc", 0x02, 0x0000, 0x00FC),
    Variant("get-config-current-sf0001-len00fc", 0x02, 0x0001, 0x00FC),
    Variant("get-config-current-sf0010-len00fc", 0x02, 0x0010, 0x00FC),
    Variant("get-config-current-sf0020-len00fc", 0x02, 0x0020, 0x00FC),
    Variant("get-config-current-sf0030-len00fc", 0x02, 0x0030, 0x00FC),
    Variant("get-config-current-sf0040-len00fc", 0x02, 0x0040, 0x00FC),
    Variant("get-config-current-sf0100-len00fc", 0x02, 0x0100, 0x00FC),
    Variant("get-config-all-sf0000-len00fc", 0x00, 0x0000, 0x00FC),
    Variant("get-config-all-sf0010-len00fc", 0x00, 0x0010, 0x00FC),
    Variant("get-config-all-sf0020-len00fc", 0x00, 0x0020, 0x00FC),
)
