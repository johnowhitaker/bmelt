#!/usr/bin/env python3
"""Read XDATA bytes through the bulk currentboot response hook.

This assumes the drive is already in currentboot and has an `xdata-cdb-bulk`
response hook installed. Each INQUIRY CDB asks the hook to copy a contiguous
XDATA chunk into response bytes starting at offset 0x20.
"""
...
def read_chunk(
    *,
    ...
    if len(proc.stdout) < end:
        raise RuntimeError(
            f"short INQUIRY response at 0x{address:04x}: need {end}, got {len(proc.stdout)}"
        )
...
def main() -> int:
    ...
    while remaining:
        ...
        if args.progress_every and len(data) % args.progress_every == 0:
            print(f"read {len(data)}/{args.length} bytes through 0x{address - 1:04x}", file=sys.stderr)
    ...
    if args.quiet:
        print(f"read {len(blob)} bytes from xdata[0x{args.address:04x}..0x{args.address + len(blob) - 1:04x}]")
    else:
